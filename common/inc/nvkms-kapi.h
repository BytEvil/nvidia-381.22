...
struct NvKmsKapiFunctionsTable {
    ...
    /*!
     * Migrate ownership from the NvKmsKapiDevice to the NVKMS device
     * specified by the (nvKmsFd, nvKmsDeviceHandle) pair.
     *
     * \param [in]  device   A device returned by allocateDevice().
     * \param [in]  nvKmsFd  File descriptor of a user-space NVKMS client.
     * \param [in]  nvKmsDeviceHandle  The device handle in nvKmsFd's namespace.
     *
     * \return NV_TRUE on success, NV_FALSE on failure.
     */
    NvBool NVKMS_KAPI_CALL (*migrateOwnership)(struct NvKmsKapiDevice *device,
                                               const NvU32 nvKmsFd,
                                               const NvU32 nvKmsDeviceHandle);
    /*!
     ...
     *
     * \param [in]  nvkmsParamsUser Userspace pointer to driver-specific surface
     *                              properties.
     *
     * \param [in]  nvkmsParamsSize Size of the memory pointed to by
     *                              nvkmsParamsUser.
     *
    ...
    (
        ...
        NvU64 nvKmsParamsUser, NvU64 nvKmsParamsSize
    );

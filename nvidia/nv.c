...
static void nv_state_set_gpu_uuid_cache(nv_state_t* nv, NvU8* uuid)
{
    if (uuid)
    {
...
static NvU8* nv_state_get_gpu_uuid_cache(nv_state_t* nv)
{
    if (nv->nv_gpu_uuid_cache.bValid)
    {
 ...
 */
static nv_linux_state_t *find_uuid(NvU8 *uuid)
{
    nv_linux_state_t *nvl = NULL;
    nv_state_t *nv;
    NvU8 *dev_uuid;
 ...
 * bug 1642200.
 */
static nv_linux_state_t *find_uuid_candidate(NvU8 *uuid)
{
    nv_linux_state_t *nvl = NULL;
    nv_state_t *nv;
    NvU8 *dev_uuid;
 ...
 * UUIDs.
 */
int nvidia_dev_get_uuid(NvU8 *uuid, nvidia_stack_t *sp)
{
    nv_state_t *nv = NULL;
    nv_linux_state_t *nvl = NULL;
    NvU8 *dev_uuid;
 ...
 */
void nvidia_dev_put_uuid(NvU8 *uuid, nvidia_stack_t *sp)
{
    nv_linux_state_t *nvl;
...
int nvidia_dev_get_pci_info(NvU8 *uuid, struct pci_dev **pci_dev_out, NvU64 *dma_start, NvU64 *dma_limit)
{
    nv_linux_state_t *nvl;
